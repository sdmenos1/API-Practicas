export default function CharacterCard({item} : {item: any}){ 
    return(
        <div key={item.id} className="border-black border rounded-lg gap-3 flex flex-col transform hover:scale-105 transition duration-300 ease-in-out pb-3 w-4/5">
                        ...
                        <div className="flex flex-col px-3">
                            ...
                            <p className="text-sm text-gray-500 text-[14px]">origin {item.origin.name}</p>
                            <p className="text-sm text-gray-500 text-[14px]">location {item.location.name}</p>
                        </div>
        </div>
    )
}